...
Crafted by IITians, for Students Everywhere
    ...
    // Option 1: Try Resend first (only works for verified emails)
    let emailResponse;
    let emailService = 'resend';
    ...
    try {
      emailResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'prepAIred Team <onboarding@resend.dev>',
          to: [email],
      ...
      // If Resend fails due to domain verification, try alternative method
      if (!emailResponse.ok) {
        const errorResponse = await emailResponse.json();
        if (errorResponse.error && errorResponse.error.includes('domain')) {
          console.log('Resend failed due to domain verification, trying alternative method');
          throw new Error('Domain not verified');
        }
      }
    } catch (resendError) {
      console.log('Resend failed, using alternative email method:', resendError.message);
      emailService = 'alternative';
      
      // Alternative: Use a simple email API service or log for manual sending
      console.log(`Would send email to: ${email}`);
      console.log('Email subject: 🎉 Welcome to prepAIred - Your AI Learning Journey Starts Here!');
      console.log('PDF attachment included: prepAIred-Welcome-Guide.pdf');
      
      // For now, we'll simulate a successful response
      // In a real scenario, you could integrate with EmailJS, SendGrid free tier, or another service
      emailResponse = {
        ok: true,
        json: async () => ({ id: 'simulated-' + Date.now(), status: 'queued' })
      };
    }
